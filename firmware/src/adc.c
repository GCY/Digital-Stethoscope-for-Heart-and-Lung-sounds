...
int16_t map_adc_pcm(uint32_t x, int16_t in_min, int16_t in_max, int16_t out_min, int16_t out_max){
   return (x - in_min) * (out_max - out_min) / (in_max - in_min) + out_min;
}
//16000hz 20Hz~800Hz
float coeff_adc[15] = {-0.029506,-0.048087,-0.041705,-0.0022188,0.064945,0.14115,0.2013,0.22412,0.2013,0.14115,0.064945,-0.0022188,-0.041705,-0.048087,-0.029506};
float buffer_adc[15];
unsigned offset_adc;
FIRInfo info_adc = {15,coeff_adc,buffer_adc,0};
...
void DMA2_Stream0_IRQHandler(void)
{
   ...
   int16_t value = map_adc_pcm(ADC3_value[0],0,4095,-32768,32767);
   int16_t MIC_Gain = 1;
   value = FIR_filter(value,&info_adc) * MIC_Gain;
   RecBuf0[0] = value;
   RecBuf1[0] = value;
}
...
void Init_ADC(void)
{
   ...
   dma.DMA_Memory0BaseAddr = (uint32_t)&ADC3_value;
   ...
   adc.ADC_NbrOfConversion = ADC_BUFFER_SIZE;
   ADC_Init(ADC3,&adc);
